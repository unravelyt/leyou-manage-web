    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="soldOutPut(props.item)">下架</v-btn>
          <v-btn icon v-else @click="soldOutPut(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      deleteItem(oldGoods){
        this.$message.confirm("删除后，不可恢复！").then(() => {
          this.$http.delete("/item/goods/spu/" + oldGoods.id).then(() => {
            this.getDataFromServer();
          })
        }).catch(() => {
      ...
      soldOutPut(oldGoods){
        this.$message.confirm("确认执行此操作吗？").then(() => {
        this.$http.put("/item/goods/spu/out/" + oldGoods.id).then(() => {
